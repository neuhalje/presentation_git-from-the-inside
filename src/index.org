...
#+REVEAL_TITLE_SLIDE: <h1>%t</h1><h2>%s</h2><h3>%A %a</h3><p>View online: <a href="%u">%u</a></p> <p><div class="r-stretch license-container"><a href="/#/slide-LICENSE"><img class="inline-icon" src="/img/cc/cc.svg"/><img class="inline-icon" src="/img/cc/by.svg"/><img class="inline-icon" src="/img/cc/sa.svg"/> <b>LICENSE</b></a></div</p><p><ul><li>Press <b>s</b> for speakers view</li><li>Press <b>o</b> for overview</li></ul></p>


#+MACRO: red @@latex:{\color{red} $1 }@@ @@html: <span class="red">$1</span> @@
#+MACRO: blue @@latex:{\color{blue} $1 }@@ @@html: <span class="blue">$1</span> @@
#+MACRO: green @@latex:{\color{green} $1 }@@ @@html: <span class="green">$1</span> @@
...
- deterministic :: $\forall data_1, data_2 \in world: data_1 == data_2 \Rightarrow h(data_1) == h(data_2)$
- collision resistant ::  $\forall data_1, data_2 \in world: h(data_1) == h(data_2) \Rightarrow data_1 == data_2$
#+END_NOTES

A /hash/ is a fixed length value derived from some data.
...
A hash /collision/ is when two different =data= values hash to the same hash.
...
For =sha1= and =git=:
- =d= - The length of the hash function ($2^{160}$)
- =n= - The total sum of all files, directories, commits, tags
...
Ergo: {{{blue(Don't create more than 10^20 (100 trillion) files and/or commits per repository.)}}}
#+HTML: </div>
...
- put(data) --> hash_{data} :: Write an object into the storage and get the objects hash back as key.
- get(hash_data) --> data :: Pass a hash and get the data. Git uses [[https://en.wikipedia.org/wiki/SHA-1][SHA-1]] as hashing function (=sha1(data) == hash_{data}=).

The main /advantages/ of such a storage is:
