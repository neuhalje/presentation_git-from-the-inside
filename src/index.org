...
function myFunction() {
  var element = document.getElementById("svg-objectstore");
  element.classList.remove("objectstore-layer-l1");
}
#+end_src

#+html:  <div id="svg-objectstore" class="scale-height objectstore-show-l1">
#+INCLUDE: "img/objectstore.svg" export html
#+html: </div>

# #+HTML: <img src="img/objectstore.svg" class="objectstore-show-l1"></img>
...
plot [1:2.5*10**20] f(x) title "probability of collision" with lines lw 3, lottery title "lottery win (1 : 139.838.160)" lw 3
#+end_src
...
**Rule of thumb:**
{{{green(Don't create more than $10^{10}$ (ten US-billion) files and $10^{10}$ commits per repository)}}}
#+HTML: </div>
#+HTML: </div> <!-- container -->
...
** Tree, Blob, Commit
:PROPERTIES:
:CUSTOM_ID: store-to-commit
:END:
#+begin_export html
<div class="r-stretch two-rows two-columns-equal">
    <div class="scale-to-cell top-left"><img class="scale-to-cell top-left" src="img/objectstore.svg"></img></div>
    <img class="scale-to-cell top-right two-rows-high" src="img/tree_complete.svg#svgView(viewBox(140,370,300,353))"></img>
    <div class="bottom-left" style="background-color: red">djfjsdjfsdfl</div>
</div>
#+end_export
** Branch
** Merge
